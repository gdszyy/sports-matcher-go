...
| v1.7 | 2026-04-17 | 细化优化完成：基于综合测试记录，在 `name.go` 引入 Jaro-Winkler 融合；在 `league_features.go` 扩充多语言序数词、预备队/州联赛识别及强约束；在 `ls_engine.go` 和 `league.go` 引入负向特征惩罚（`CalcFeaturePenalty`）和地理别名词典（`geoSimilarity`）；新增 9 个单元测试并全部通过。 | Manus AI |
